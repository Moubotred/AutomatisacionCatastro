...
def ModeloNeuronal(Modelo,Etiquetas,Ruta,Archivo):
    # Disable scientific notation for clarity
    np.set_printoptions(suppress=True)

    # Load the model
    model = load_model(os.path.join(os.getcwd(),'__modelos__',f'{Ruta}',f"{Modelo}.h5"), compile=False)

    # Load the labels
    class_names = open(os.path.join(os.getcwd(),'__modelos__',f'{Ruta}',f"{Etiquetas}.txt"), "r").readlines()
