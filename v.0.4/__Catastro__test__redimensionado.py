...
class app:
    ...
    def ExtraccionInformacion(self):
    ...
    def ResolverPregunta(self,inidice_pregunta):
        if inidice_pregunta == 0:
            response1 = Ts.spRealizarCatastro(len(os.listdir(os.path.join(os.getcwd(),'__fotos_sad__'))))
            return response1
        
        if inidice_pregunta == 1:
            response2 = Ts.spTipoSuministro(os.path.join(os.getcwd(),'__fotos_sad__','Imagen_003.jpg'))
            return response2

        if inidice_pregunta == 2:
            response1 = Ts.spTecnologiaMedidor(os.path.join(os.getcwd(),'__fotos_sad__','Imagen_003.jpg'))
            return response1
        
        if inidice_pregunta == 3:
            response2 = Ts.spTipoConexion()
            return response2
        
        try:
            if inidice_pregunta == 4:
                response1 = Ts.spTipoSuperficie()
                return response1
            
        except NoSuchElementException:
            response1 = Ts.spTipoRedAerea()
            Pa = self.wait.until(EC.presence_of_element_located((MobileBy.ID,'com.luzdelsur.bt.inspecciones.ui:id/spTipoRedAerea'))).click()
            Ra = self.wait.until(EC.presence_of_element_located((MobileBy.XPATH, '//android.widget.CheckedTextView[@resource-id="android:id/text1" and @text="$"]'.replace('$',f'{response1}')))).click() 
            
    
        if inidice_pregunta == 5:
            response2 = Ts.spUbicacionConexion()
            return response2
        
        try:
            if inidice_pregunta == 6:
                response1 = Ts.spRotulado(os.path.join(os.getcwd(),'__fotos_sad__','Imagen_005.jpg'),suministro=self.conjuntosuministro)
                return response1
            
        except TimeoutException:
            response1 = Ts.spTipoMastil()
            Pa = self.wait.until(EC.presence_of_element_located((MobileBy.ID,'com.luzdelsur.bt.inspecciones.ui:id/spTipoMastil'))).click()
            Ra = self.wait.until(EC.presence_of_element_located((MobileBy.XPATH, '//android.widget.CheckedTextView[@resource-id="android:id/text1" and @text="$"]'.replace('$',f'{response1}')))).click()
        
        if inidice_pregunta == 7:
            response2 = Ts.edtElectrIniCMedicion()
            return response2

    def AccionesPreguntas(self,indice_pregunta,enviar_texto=None):
        """
        ...
        if indice_pregunta is not None:
            ...
            Post = self.ResolverPregunta(indice_pregunta)

            # resolucion = '//android.widget.CheckedTextView[@resource-id="android:id/text1" and @text="$"]'.replace('$',f'{Post}')
            # print(resolucion)

            Ra = wait.until(EC.presence_of_element_located((MobileBy.XPATH, '//android.widget.CheckedTextView[@resource-id="android:id/text1" and @text="$"]'.replace('$',f'{Post}')))).click() 
    ...
    def CatastroPreguntas_widget(self,widget):
        ...
        try:    
            ...
            self.AccionesPreguntas(0)

            self.Scroll(scrollview,self.scroll_points[0],-140)

            self.AccionesPreguntas(1)
            self.AccionesPreguntas(2)
            self.AccionesPreguntas(3)

            self.Scroll(scrollview,self.scroll_points[1],-140)

            self.AccionesPreguntas(4)
            self.AccionesPreguntas(5)

            self.Scroll(scrollview,self.scroll_points[2],-140)

            self.AccionesPreguntas(6)
    ...
    def BuscarSuministro(self,conjunto_suministro): 
        ...
        self.conjuntosuministro = conjunto_suministro
